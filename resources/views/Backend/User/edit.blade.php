...
<body>
<div class="container">
<form action="{{route('user.update',$allContent->id)}}" method="post" id="user_add">
    @csrf
    {{ method_field('PUT') }}
    <h1>Update User</h1>
    ...
    <div class="form-check">
      <input class="form-check-input" type="radio" name="gender" id="exampleRadios1" value="male" @if ($allContent->gender=='male'){{ "checked=checked" }}  @endif>
      <label class="form-check-label" for="gender">
      Male
      </label>
      <input class="form-check-input" type="radio" name="gender" id="exampleRadios2" value="female" @if ($allContent->gender=='female') {{ "checked=checked" }}  @endif>
      <label class="form-check-label" for="gender">
      Female
      </label>
    </div>
